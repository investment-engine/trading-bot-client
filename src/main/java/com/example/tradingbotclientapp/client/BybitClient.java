...
public class BybitClient {
    ...
    public BybitClient(BybitConfig bybitConfig) {
        ...
        this.clientTrade = BybitApiClientFactory.newInstance(bybitConfig.getApiToken(),
                bybitConfig.getSecretKey(), bybitConfig.getBaseUrl(), true).newTradeRestClient();
        this.accountClient = BybitApiClientFactory.newInstance(bybitConfig.getApiToken(),
                bybitConfig.getSecretKey(), bybitConfig.getBaseUrl()).newAccountRestClient();

    }

    public BybitWalletBalance getWalletInfo(HuobiSymbol symbol) throws JsonProcessingException {
    ...
    public void closePosition(HuobiSymbol symbol, BigDecimal qty) {
        log.info("Trying CLOSE position for PAIR: " + symbol.toString());
        var newOrderRequest = TradeOrderRequest.builder().category(CategoryType.SPOT).symbol(symbol.getDescription())
        ...
        log.info("Responce: " + resp);
    }

    public void openPosition(HuobiSymbol symbol, BigDecimal qty) {
        log.info("Trying open position for PAIR: " + symbol.toString());
        var newOrderRequest = TradeOrderRequest.builder().category(CategoryType.SPOT).symbol(symbol.getDescription())
